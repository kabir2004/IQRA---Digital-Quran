...
export default function HizbReadingPage() {
  ...
  const hizbNumber = parseInt(params.hizb as string)
  
  const [hizb, setHizb] = useState<Hizb | null>(null)
  ...
  useEffect(() => {
    const loadHizbData = async () => {
      try {
        ...
        const hizbList = await dataAdapter.getHizb()
        const currentHizb = hizbList.find(h => h.index === hizbNumber)
        ...
        // Load ayahs for the range covered by this Hizb (across multiple surahs if needed)
        const ayahsData = await dataAdapter.getHizbAyahs(hizbNumber)
        const translationsData = await dataAdapter.getHizbTranslations(hizbNumber, 'en')
        const transliterationsData = await dataAdapter.getHizbTransliterations(hizbNumber, 'en')
  ...
  const navigateToHizb = (direction: 'prev' | 'next') => {
    ...
    if (newHizb >= 1 && newHizb <= 120) { // Assuming 120 total Hizb sections
      router.push(`/read/hizb/${newHizb}`)
    }
  }
      ...
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center">
            <p className="text-muted-foreground">Loading Hizb...</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-muted-foreground">Hizb not found</p>
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Reading Navigation */}
      <ReadingNavigation currentType="hizb" currentId={hizbNumber} />
          ...
          <div className="text-center">
            <div className="flex items-center justify-center gap-4 mb-2">
              <Button
                variant="ghost"
                size="sm"
                onClick={() => navigateToHizb('prev')}
                disabled={hizbNumber <= 1}
              >
              ...
              <div>
                <div className="flex items-center justify-center gap-2 mb-1">
                  ...
                  <h1 className="text-2xl font-bold text-foreground">Hizb {hizb.index}</h1>
                </div>
                <p className="text-lg text-foreground">{hizb.name}</p>
                ...
                <div className="flex items-center justify-center gap-4 mt-2 text-sm text-muted-foreground">
                  <span>From: {hizb.start_surah}:{hizb.start_ayah}</span>
                  <span>To: {hizb.end_surah}:{hizb.end_ayah}</span>
                </div>
              </div>
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => navigateToHizb('next')}
                disabled={hizbNumber >= 120}
              >
